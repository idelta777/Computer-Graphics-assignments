...
public final class Sliders extends JFrame implements ActionListener{
   private JSlider slider;private JSlider slidera;
   private JSlider sliderb;private JSlider sliderc;
   private SpiroSlaider miPanel;
   Container contenedor = getContentPane();
   JButton boton1;JButton boton2;JButton boton3;
   JButton boton4;JButton boton5;JButton boton6;
   
   public static void main( String args[] ){
      Sliders aplicacion = new Sliders();
      aplicacion.setDefaultCloseOperation( JFrame.EXIT_ON_CLOSE );
   }
   
   public Sliders() {
      super("Spirograph");
      miPanel = new SpiroSlaider();
      miPanel.setBackground( Color.RED );
      slider = new JSlider( SwingConstants.HORIZONTAL, 0, 10000, 100 );
      slider.addChangeListener(new ChangeListener() {
            @Override
             public void stateChanged( ChangeEvent e ){
               miPanel.VarPuntos( slider.getValue() );
             }}); 
      ...
      sliderc.addChangeListener(new ChangeListener() {
          @Override
             public void stateChanged( ChangeEvent e ){
               miPanel.c( sliderc.getValue() );
             }}); 
      contenedor.add( sliderc, BorderLayout.NORTH );
      botones();
      sliderc.setVisible(false);setVisible( true );
    }
   public void botones(){
   ...
   public void actionPerformed(ActionEvent e) {
       if(e.getSource()==boton1)
        {sliderc.setVisible(false);miPanel.bandera(1);}
       if(e.getSource()==boton2)
        {sliderc.setVisible(false);miPanel.bandera(2);}
       if(e.getSource()==boton3)
        { sliderc.setVisible(true);miPanel.bandera(3);}
       if(e.getSource()==boton4)
        {sliderc.setVisible(false);miPanel.bandera(4);}
       if(e.getSource()==boton5)
        {sliderc.setVisible(false);miPanel.bandera(5);}
       if(e.getSource()==boton6)
        {sliderc.setVisible(false);miPanel.bandera(6);}
   } 
}


class SpiroSlaider extends JPanel {
  int nPoints = 1;
  double a = 67;
  double b = 53;
  double c = 100;
